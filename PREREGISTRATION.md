...
> 以下是**看到資料之後**才產生的想法。與預先登錄的 H1/H2/H0 分開呈現，
> 報告中必須明確標示為探索性分析，不得當作預先登錄的檢定。

### P1 · 2026-09-01 — 序列長度作為候選機制（源自 T3 空結果 + T4 fertility）
T3 顯示權重空間的量化誤差與語言、頻率皆無關（相對誤差近乎常數 0.093）。
因此若 T5／T8 觀察到行為層的不對稱，它**不能**用「低資源 token 權重誤差較大」解釋。

一個由本研究自身資料生出的替代機制：T4 顯示同一句話在緬甸語需要英語 5.04 倍的 token。
更多 token = 更多次前向傳遞 = 量化誤差有更多次累積機會。
**預測**：控制住 tier 之後，`fertility` 仍應與退化幅度正相關。
主迴歸已包含 `fertility` 項，因此此預測可由既有分析檢視，但其**因果詮釋屬探索性**。

### P2 · 2026-09-01 — GQA 的 KV 投影是誤差熱點（源自 T3 逐層分析）
`v_proj`／`k_proj`（GQA 壓縮到 4 個 KV head 的 512 維矩陣）量化誤差最高，
`v_proj` 深層中位 0.1020、最大 0.1245，明顯高於 MLP 的 ≈0.092。
**可做的實驗**：一個「arm E'」把 `k_proj`/`v_proj` 保留 8-bit（參數量極小，記憶體代價遠低於 arm E）。
**列為選作**，優先度低於既有五個 arm，僅在 Day 7 中場關卡有餘裕時執行。
