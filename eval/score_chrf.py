...
# mlx-lm emitted the stop token <|END_RESPONSE|> as text in 5329 of 5400 generations. No text
# followed it, so stripping is lossless, but leaving it in depressed every score.
SPECIAL = re.compile(r"<\|[^|]*\|>")
clean = lambda t: SPECIAL.sub("", t).strip()
...
for arm in ARMS:
    for lang in LANGS:
        ...
        hyps = [clean(x["hyp"]) for x in d]; refs = [[x["ref"] for x in d]]
